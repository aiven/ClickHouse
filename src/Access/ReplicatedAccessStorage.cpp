...
{
    main_thread = ThreadFromGlobalPool(&ReplicatedAccessStorage::runMainThread, this);
}
...
UUID ReplicatedAccessStorage::insertImpl(const AccessEntityPtr & new_entity, bool replace_if_exists)
{
    ...
    {
        if (tryInsertNoLock(id, new_entity, replace_if_exists, notifications))
            return id;
    }

    throw Exception(ErrorCodes::UNEXPECTED_ZOOKEEPER_ERROR, "Cannot insert access entity");
}


bool ReplicatedAccessStorage::tryInsertNoLock(const UUID & id, const AccessEntityPtr & new_entity, bool replace_if_exists, Notifications & notifications)
{
    const String & name = new_entity->getName();
    ...
    if (res == Coordination::Error::ZNODEEXISTS)
    {
        ...
        if (responses[0]->error == Coordination::Error::ZNODEEXISTS)
        {
            ...
            String existing_entity_definition;
            const bool exists = zookeeper->tryGet(entity_path, existing_entity_definition);
            if (!exists)
                return false; // Retry

            AccessEntityPtr existing_entity = parseAccessEntity(existing_entity_definition, "znode", entity_path);
        ...
        else if (replace_if_exists)
        {
            ...
            /// If that happens, then we'll just retry from the start.
            const bool uuid_exists = zookeeper->tryGet(name_path, existing_entity_uuid, &name_stat);
            if (!uuid_exists)
                return false; // Retry

            /// Then get the definition
            const String existing_entity_path = zookeeper_path + "/uuid/" + existing_entity_uuid;
            String existing_entity;
            Coordination::Stat entity_stat;
            const bool name_exists = zookeeper->tryGet(existing_entity_path, existing_entity, &entity_stat);
            if (!name_exists)
                return false; // Retry

            Coordination::Requests replace_ops;
            replace_ops.emplace_back(zkutil::makeRemoveRequest(existing_entity_path, entity_stat.version));
            ...
            Coordination::Responses replace_responses;
            const auto replace_res = zookeeper->tryMulti(replace_ops, replace_responses);
            if (replace_res != Coordination::Error::ZOK)
                return false; // Retry
        }
        else
        {
    ...
    else if (res != Coordination::Error::ZOK)
    {
...
void ReplicatedAccessStorage::removeImpl(const UUID & id)
{
    ...
    for (size_t retry=0; retry<10; ++retry)
    {
        if (tryRemoveNoLock(id, notifications))
            return;
    }
    throw Exception(ErrorCodes::UNEXPECTED_ZOOKEEPER_ERROR, "Cannot remove access entity");
}


bool ReplicatedAccessStorage::tryRemoveNoLock(const UUID & id, Notifications & notifications)
{
    auto zookeeper = get_zookeeper();
    const String entity_uuid = toString(id);
    const String entity_path = zookeeper_path + "/uuid/" + entity_uuid;
    ...
    const AccessEntityPtr entity = parseAccessEntity(entity_definition, "znode", entity_path);
    ...
    Coordination::Responses responses;
    const auto res = zookeeper->tryMulti(ops, responses);
    if (res == Coordination::Error::ZBADVERSION || res == Coordination::Error::ZNONODE)
        return false; // Retry
...
void ReplicatedAccessStorage::updateImpl(const UUID & id, const UpdateFunc & update_func)
{
    ...
    for (size_t retry=0; retry<10; ++retry)
    {
        if (tryUpdateNoLock(id, update_func, notifications))
            return;
    }
    throw Exception(ErrorCodes::UNEXPECTED_ZOOKEEPER_ERROR, "Cannot update access entity");
}


bool ReplicatedAccessStorage::tryUpdateNoLock(const UUID & id, const UpdateFunc & update_func, Notifications & notifications)
{
    auto zookeeper = get_zookeeper();
    const String entity_uuid = toString(id);
    const String entity_path = zookeeper_path + "/uuid/" + entity_uuid;
    ...
    const auto uuid_exists = zookeeper->tryGet(entity_path, old_entity_definition, &stat);
    if (!uuid_exists)
        throwNotFound(id);

    const AccessEntityPtr old_entity = parseAccessEntity(old_entity_definition, "znode", entity_path);
    ...
    else if (res == Coordination::Error::ZBADVERSION)
    {
        return false; // Retry
    }
    else if (res != Coordination::Error::ZOK)
...
void ReplicatedAccessStorage::runMainThread()
{
    ...
    while (!stop_flag)
    {
        try
        {
            if (!initialized)
            {
                ...
                LOG_DEBUG(getLogger(), "Initialized worker thread");
            }

            LOG_DEBUG(getLogger(), "Waiting for refresh");
            refresh_event->wait();
            refresh();
        }
        catch (...)
        {
            tryLogCurrentException(getLogger(), "Unexpected error, will try to restart main thread:");
            sleepForSeconds(5);
        }
    }
}

void ReplicatedAccessStorage::initializeMainThread()
{
    assert(!initialized);
    auto zookeeper = get_zookeeper();
...
void ReplicatedAccessStorage::createRootNodes(const zkutil::ZooKeeperPtr & zookeeper)
{
    ...
    for (auto type : ext::range(IAccessEntity::Type::MAX))
    {
        /// Create a znode for each type of AccessEntity
        auto type_info = IAccessEntity::TypeInfo::get(type);
...
void ReplicatedAccessStorage::refreshEntityNoLock(
    ...
    Notifications & notifications)
{
    ...
    auto watch_entity = [this, id](const Coordination::WatchResponse & response) {
    ...
    if (exists)
    {
        auto entity = parseAccessEntity(entity_definition, "znode", entity_path);
...
ext::scope_guard ReplicatedAccessStorage::subscribeForChangesImpl(EntityType type, const OnChangedHandler & handler) const
{
    std::lock_guard lock{mutex};
...
ext::scope_guard ReplicatedAccessStorage::subscribeForChangesImpl(const UUID & id, const OnChangedHandler & handler) const
{
    std::lock_guard lock{mutex};
    const auto it = entries_by_id.find(id);
    if (it == entries_by_id.end())
