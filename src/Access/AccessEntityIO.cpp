...
AccessEntityPtr parseAccessEntity(const String & definition, const String & source_type, const String & source)
{
    ...
    for (const auto & query : queries)
    {
        ...
        {
            if (res)
                throw Exception("Two access entities in one " + source_type + " " + source, ErrorCodes::INCORRECT_ACCESS_ENTITY_DEFINITION);
        ...
        {
            if (res)
                throw Exception("Two access entities are attached in the same " + source_type + " " + source, ErrorCodes::INCORRECT_ACCESS_ENTITY_DEFINITION);
        ...
        else if (auto * grant_query = query->as<ASTGrantQuery>())
        {
            if (!user && !role)
                throw Exception(
                    "A user or role should be attached before grant in " + source_type + " " + source, ErrorCodes::INCORRECT_ACCESS_ENTITY_DEFINITION);
            if (user)
    ...
    if (!res)
        throw Exception("No access entities attached in " + source_type + " " + source, ErrorCodes::INCORRECT_ACCESS_ENTITY_DEFINITION);
