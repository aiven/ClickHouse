...
/// NOTE(aiven): upstream master also handles key-value asynchronous metrics here, writing one line per key
/// with the key as a label (`ClickHouseAsyncMetrics_BlockReadBytes{device="sda"} 123`) and therefore taking
/// the constant labels both with and without braces. On 26.3 `AsynchronousMetricValue` is a plain scalar -
/// no `isMap`, no `key_label`, no `key_values` - so only the scalar path exists and the braced suffix is
/// enough.
...
void PrometheusMetricsWriter::writeDimensionalMetric(WriteBuffer & wb, const DimensionalMetrics::MetricFamily & family, const std::string & extra_labels)
{
    ...
    writeOutLine(wb, "# HELP", base_name, help_text);
    /// NOTE(aiven): upstream master writes `family.getTypeString` here, because a dimensional metric family
    /// can be a counter as well as a gauge. On 26.3 `DimensionalMetrics::MetricFamily` has no metric type, so
    /// every dimensional family is exposed as a gauge, as it was before that upstream change.
    ...
    bool expose_dimensional_metrics) const
{
    ...
    /// NOTE(aiven): upstream master reserves `channel`, `cpu`, `device`, `disk`, `interface`, `mc` and
    /// `sensor` here when `expose_asynchronous_metrics` is on and `asynchronous_metrics_key_values_mode` is
    /// not `legacy_names`, because those are the key labels of its key-value asynchronous metrics. On 26.3
    /// asynchronous metrics are always scalars and never write a label, so nothing is reserved for them and
    /// those names remain usable as constant labels.
    ...
    [[maybe_unused]] bool expose_dimensional_metrics) const
{
    ...
    /// NOTE(aiven): as in the base class, upstream master also reserves the key labels of the key-value
    /// asynchronous metrics here; on 26.3 there are none, so `expose_asynchronous_metrics` and
    /// `async_metrics_mode` are unused.
