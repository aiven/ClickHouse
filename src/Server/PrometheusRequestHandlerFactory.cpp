...
namespace
{
    ...
    /// NOTE(aiven): upstream master takes a trailing `const std::optional<String> & default_session_user`
    /// here and copies it into `parsed_config.connection_config.default_session_user` for every parsed
    /// handler. On 26.3 that field - and the whole `default_session_user` feature behind it - does not exist,
    /// so the parameter is dropped; see the comment in `PrometheusRequestHandlerFactory.h`.
