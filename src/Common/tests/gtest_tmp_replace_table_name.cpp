...
{
    auto result = DB::TemporaryReplaceTableName::fromString(".tmp_replace_efe23F_wef44wE");
...
{
    auto result = DB::TemporaryReplaceTableName::fromString(".tmp_replace_efe23F_");
    ASSERT_FALSE(result.has_value());
}

TEST(TemporaryReplaceTableName, FromStringMissingNameHash)
{
    auto result = DB::TemporaryReplaceTableName::fromString(".tmp_replace__wef44we");
    ASSERT_FALSE(result.has_value());
}
...
TEST(TemporaryReplaceTableName, ToStringValidInput)
{
    ...
    EXPECT_EQ(t.toString(), ".tmp_replace_abc123_42abc");
}
