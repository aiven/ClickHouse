...
public:
    ...
    /// NOTE(aiven): upstream master overrides the pure-virtual `mergeImpl`, behind a non-virtual `merge`
    /// wrapper that asserts `place != rhs`. On 26.3 `merge` is still the pure virtual itself; see the same
    /// note in `AggregateFunctionTimeseriesBase.h`.
