...
public:
    ...
    /// NOTE(aiven): upstream master overrides `IAggregateFunction::shouldPrintParametersWithTypes` here to
    /// return true, so that `DataTypeAggregateFunction::getNameImpl` prints the `start`, `end`, `step` and
    /// `window` parameters through `FieldVisitorToCastedLiteral` with `::Type` suffixes - a `DateTime64`
    /// start coming from a `toDateTime64` cast is a `DecimalField`, whose plain printed form is
    /// indistinguishable from a `String` literal. On 26.3 neither that virtual nor
    /// `FieldVisitorToCastedLiteral` exists, and adding them means porting a new `Field` visitor and
    /// rewiring `getNameImpl`, so the override is dropped. The untyped form still round-trips, because
    /// `parseTimeSeriesTimestamp` and `parseTimeSeriesDuration` accept a `String` field and the parameter is
    /// rescaled to the timestamp column's scale anyway; the only thing lost is the parameter's original
    /// decimal scale in the printed type name.
    ...
    /// NOTE(aiven): upstream master also overrides `IAggregateFunction::addBatchWithNonNullPlaces` here, an
    /// `addBatch` variant that callers use when every entry of `places` is guaranteed non-null. On 26.3 that
    /// virtual does not exist and nothing calls it, so the override is dropped: `addBatch` above is the only
    /// batch entry point in 26.3 and already dispatches both the array and the scalar argument shapes.
    ...
    /// NOTE(aiven): upstream master split `IAggregateFunction::merge` into a non-virtual public entry point
    /// that asserts `place != rhs` and a pure-virtual `mergeImpl` that implementations override. On 26.3
    /// `merge` itself is still the pure virtual, and adopting upstream's shape would mean renaming the
    /// override in 88 places across 70 files, so the body stays on `merge`.
