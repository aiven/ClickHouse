...
public:
    ...
    /// NOTE(aiven): upstream master has these four `ASTPtr`-taking overloads; 26.3 only has the
    /// `T * &` forms. `ASTViewTargets` needs them to register `inner_engine`, `inner_columns` and
    /// `table_ast` as real children. No overload ambiguity: the existing forms take `T * &`, which
    /// never matches `boost::intrusive_ptr<IAST> &`.
