...
public:
    ...
    /// NOTE(aiven): upstream master also declares `writeJSON` and `readJSON` here, for the AST-JSON
    /// (de)serialization subsystem. On 26.3 that subsystem does not exist at all - there is no
    /// `src/Parsers/ASTJSONHelpers.h`, no `IAST::writeJSON`/`readJSON` virtuals and no
    /// `IAST::createFromJSON` - so the JSON half of upstream's file is omitted here. Consequently
    /// `formatTarget` takes `time_series_version` explicitly, since upstream's
    /// `writeJSON(WriteBuffer &, std::optional<UInt64>)` overload, which carries the same argument
    /// for the JSON path, has nothing to attach to.
