...
CreateQueryUUIDs::CreateQueryUUIDs(const ASTCreateQuery & query, bool generate_random, bool for_restore)
{
    ...
    if (generate_random)
    {
        ...
        if (!query.attach)
        {
            ...
            /// NOTE(aiven): upstream master has `ASTRefreshStrategy::isAppend`, added together with the
            /// `RefreshMode` enum (`Replace` / `AppendFull` / `AppendIncremental`). On 26.3 the strategy
            /// still carries a single `bool append`, so the flag is read directly; the condition is the same.
