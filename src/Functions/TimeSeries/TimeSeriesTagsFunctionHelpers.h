...
/// NOTE(aiven): upstream master forward-declares `ColumnsWithTypeAndName` as
/// `VectorWithMemoryTracking<ColumnWithTypeAndName>`. On 26.3 it is a plain `std::vector`, so
/// re-declaring it here is an ODR conflict; include the canonical declaration instead.
...
/// Group is an integer (UInt64) which is used to identify a set of tags.
VectorWithMemoryTracking<Group> extractGroupFromArgument(std::string_view function_name, const ColumnsWithTypeAndName & arguments, size_t argument_index, bool return_single_element_if_const_column = false);
