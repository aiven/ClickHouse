...
#define MERGE_TREE_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(UInt64, queue_size_to_delay_insert, 50000, "If replication queue is at least that large on any replica, artificially slow down insert into table.", 0) \
    DECLARE(UInt64, queues_total_size_to_delay_insert, 100000, "If the sum, for all tables, of the largest replication queue's size over all replicas is larger than this value, artificially slow down insert into table.", 0) \
    ...
    DECLARE(UInt64, queue_size_to_throw_insert, 100000, "If replication queue is at least that large on any replica, throw 'Too large replication queue ...' exception", 0) \
    DECLARE(UInt64, queues_total_size_to_throw_insert, 200000, "If the sum, for all tables, of the largest replication queue's size over all replicas is larger than this value, throw 'Too large replication queue ...' exception.", 0) \
    ...
    DECLARE(Bool, use_early_fetch_pool, false, "use a separate pool for early fetches", 0) \
