    ...
    bool validate_substitutions)
{
    ...
    auto expand_macro = [&] (ASTLiteral * ast_zk_path, ASTLiteral * ast_replica_name, String zookeeper_path, String replica_name) -> TableZnodeInfo
    {
        ...
        /// dossier 058).
        /// `validate_substitutions` additionally restricts the check to a freshly supplied
        /// definition (`CREATE`, full-definition `ATTACH`), the same scope upstream introduced the
        /// flag for in `617b6626fcf`. Every other route only re-derives a table that already
        /// exists - server startup, `RESTORE`, and `extractZooKeeperPathFromReplicatedTableDef`,
        /// which swallows `BAD_ARGUMENTS` into `nullopt` - so rejecting there would fail to load
        /// or silently drop from a backup a table whose path predates the enforcement.
        if (validate_substitutions && server_settings[ServerSetting::aiven_enforce_default_replication_path])
        {
