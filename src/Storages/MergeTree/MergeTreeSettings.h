...
#define MERGE_TREE_SETTINGS(M, ALIAS) \
    ...
    M(UInt64, queue_size_to_delay_insert, 50000, "If replication queue is at least that large on any replica, artificially slow down insert into table.", 0) \
    ...
    M(UInt64, queue_size_to_throw_insert, 100000, "If replication queue is at least that large on any replica, throw 'Too large replication queue ...' exception", 0) \
    ...
    M(Bool, use_early_fetch_pool, false, "use a separate pool for early fetches", 0) \
