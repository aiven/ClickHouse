...
namespace FailPoints
{
    ...
    /// Makes the (retained, off-the-main-path) tryBecomeGlobalLeader helper lose the race for the
    /// 'running' znode, to test the coordinated shard-leader recovery path.
        ...
        else
        {
            ...
            if (coordination.root_znode.refresh_running)
            {
                         ...
                         && !view->getContext()->getRefreshSet().refreshesStopped())
                {
                    ...
                    String shard_log_comment = fmt::format("refresh of {}", view->getStorageID().getFullTableName());
                    std::vector<StorageID> shard_deps = set_handle.getDependencies();
                    lock.unlock();
                    try
                    {
                        if (tryBecomeShardLeader(zookeeper))
                        {
                            ...
                            {
                                /// Only the global leader writes the coordination znode, so the cursor this
                                /// shard's insert advanced to is discarded here (incremental refresh is not
                                /// supported across shards; see executeRefreshUnlocked).
                                CursorTreeNodePtr discarded_cursor;
                                executeRefreshUnlocked(temp_table_id, shard_deps, shard_log_comment, discarded_cursor);
...
void RefreshTask::executeRefresh()
{
    ...
    /// Captured under the lock because the refresh below runs with `mutex` released and the
    /// shard-coordination helpers read them from this thread.
    ...
    try
    {
        ...
        StorageID target_table = view->prepareTableForInsert(refresh_mode, create_context);
        if (!append)
        ...
        new_table_uuid = executeRefreshUnlocked(target_table, deps, log_comment, cursor_after_refresh);
        ...
        exchangeTargetTableAfterRefresh(target_table, root_znode_version);
...
UUID RefreshTask::executeRefreshUnlocked(const StorageID & target_table_id, const std::vector<StorageID> & deps, const String & log_comment, CursorTreeNodePtr & out_cursor)
{
    ...
    /// The thread group must stay attached for the whole query, so it lives here rather than inside
    /// the try (prepareRefresh hands it back together with the insert query).
    ...
    /// The placeholder text is load-bearing: `03704_rmv_logging_internal_queries` asserts an
    /// `ExceptionBeforeStart` row whose `query` is exactly `(create target table)`.
    ...
    try
    {
        ...
        /// The table to insert into was already created by the caller (executeRefresh, via
        /// prepareTableForInsert) and, in a coordinated refresh, is the same table on every shard.
        std::tie(refresh_query, query_scope) = view->prepareRefresh(refresh_mode, refresh_context, target_table_id, stream_cursor);
    ...
    catch (...)
    {
        ...
        {
            /// Failed when preparing the query, or after it already finished.
...
void RefreshTask::exchangeTargetTableAfterRefresh(const StorageID & target_table_id, int32_t root_znode_version)
{
    if (isAppend())
        return;
    ...
    /// The executor is gone once the insert pipeline's block in executeRefreshUnlocked returns, so
    /// interruptExecution() is a no-op past this point. The exchange is the destructive coordinated
    /// step, so re-check the interrupt flag here: a cancellation that lands in this post-pipeline
    /// window must still skip the exchange, not swap the target table after the refresh was cancelled.
    FailPointInjection::pauseFailPoint(FailPoints::refresh_mv_pause_before_exchange);
    {
        std::unique_lock exec_lock(execution.executor_mutex);
        if (execution.interrupt_execution.load())
            throw Exception(ErrorCodes::QUERY_WAS_CANCELLED, "Refresh for view {} cancelled", view->getStorageID().getFullTableName());
    }
...
String RefreshTask::CoordinationZnode::toString() const
{
    ...
    out << "refresh_dir: " << escape << refresh_dir << "\n"
...
void RefreshTask::CoordinationZnode::parse(const String & data, bool running_znode_exists, const LoggerPtr & log_)
{
    *this = {};
    ...
    optional_field("refresh_dir", refresh_dir);
    optional_field("target_table_id", target_table_id);
