...
void RefreshTask::readZnodesIfNeeded(std::shared_ptr<zkutil::ZooKeeper> zookeeper, std::unique_lock<std::mutex> & lock)
{
    ...
    /// Read the znodes. This is an atomic multi-read where Keeper supports it, and separate
    /// per-path reads otherwise; the reads do not have to be atomic (see the commit message).
