...
public:
    ...
    /// will actually drop.
    /// NOTE(aiven): upstream master renamed this hook to `checkTableSizeBelowDropLimit`;
    /// on 26.3 the `IStorage` hook is still called `checkTableCanBeDropped`.
    ...
    /// NOTE(aiven): upstream master added a fourth `DDLGuardPtr &` parameter to `IStorage::alter`;
    /// 26.3 still takes three.
