...
void KafkaConsumer::createConsumer(cppkafka::Configuration consumer_config, UInt64 auto_offset_reset_ms_)
{
    ...
    // NOTE: cppkafka calls Consumer::assign(topic_partitions) AFTER this callback returns,
    // using the same vector (see contrib/cppkafka Consumer::handle_rebalance). So to override
    // offsets we must mutate topic_partitions in place here (do NOT call consumer->assign()
    // from inside this callback, it would be overwritten).
