...
#define KAFKA_RELATED_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(String, kafka_ssl_endpoint_identification_algorithm, "", "Endpoint identification algorithm used to validate the broker hostname against the broker certificate. Supported: empty string, none, https. In case of empty string the value is not set by the table, thus values from the config files or the default value from `librdkafka` (`https`) will be used.", 0) \
    ...
    DECLARE(String, kafka_partition_shard_num, "", "The shard number of this instance for static partition-to-shard affinity. Must be between 1 and kafka_shard_count inclusive. Supports macro expansion (e.g. '{shard}'). Must be used together with kafka_shard_count.", 0) \
