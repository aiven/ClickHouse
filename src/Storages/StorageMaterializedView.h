...
private:
    ...
    StorageID prepareTableForInsert(bool append, ContextMutablePtr refresh_context) const;
    std::tuple<std::shared_ptr<ASTInsertQuery>, std::unique_ptr<CurrentThread::QueryScope>>
    prepareRefresh(ContextMutablePtr refresh_context, StorageID target_table) const;
