...
namespace
{
    ...
    void removeGeneratedInnerEngine(ASTStorage & inner_engine, ViewTarget::Kind inner_table_kind, const TimeSeriesSettings & settings)
    {
        ...
        switch (inner_table_kind)
        {
            ...
            case ViewTarget::Tags:
            {
                ...
                /// NOTE(aiven): upstream master also removes `allow_dimensions_outside_sorting_key` here; on
                /// 26.3 that setting is never emitted (see the note further down), so there is nothing to remove.
                remove_settings({
        ...
        const Settings & query_settings)
    {
        ...
        auto set_engine = [&](std::string_view engine_kind)
        {
            ...
            /// Formats as `MergeTree` instead of `MergeTree()`, the same way the parser reads an engine without arguments.
        ...
        switch (inner_table_kind)
        {
            ...
            case ViewTarget::Tags:
            {
                ...
                /// NOTE(aiven): the TimeSeries `tags` inner table keeps the tag columns (and the `tags` Map)
                /// outside the sorting key. Upstream master's `AggregatingMergeTree` rejects that layout unless
                /// the `allow_dimensions_outside_sorting_key` setting is on, so upstream sets it here. On 26.3
                /// neither the setting nor the rejection exists, and setting it makes `CREATE TABLE ... ENGINE =
                /// TimeSeries` fail outright with `UNKNOWN_SETTING`, so it is not emitted. The layout itself is
                /// unchanged and remains safe: the off-key columns are functionally dependent on `id`, which is
                /// in the sorting key, so every group of rows a merge collapses shares their values.
                break;
            }
...
void normalizeTimeSeriesDefinitionImpl(ASTCreateQuery & create_query, const NormalizeTimeSeriesDefinitionParams & params)
{
    ...
    if (is_new_table && !create_query.as_table.empty())
    {
        ...
        old_create_query = normalizeASCreateQuery(*params.as_create_query);
