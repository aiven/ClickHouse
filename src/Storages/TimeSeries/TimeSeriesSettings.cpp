...
/// NOTE(aiven): upstream master's `DECLARE_SETTINGS_TRAITS` / `IMPLEMENT_SETTINGS_TRAITS` take extra
/// arguments and auto-generate both the `Impl` struct and the `TimeSeriesSetting` namespace of
/// `SettingIndex` externs. On 26.3 both macros take two arguments and that boilerplate is written out
/// by hand, as in `src/Databases/DatabaseReplicatedSettings.cpp`.
...
#define INITIALIZE_SETTING_EXTERN(TYPE, NAME, DEFAULT, DESCRIPTION, FLAGS, ...) \
    TimeSeriesSettings##TYPE NAME = &TimeSeriesSettingsImpl ::NAME;

namespace TimeSeriesSetting
{
LIST_OF_TIME_SERIES_SETTINGS(INITIALIZE_SETTING_EXTERN, INITIALIZE_SETTING_EXTERN)
}
