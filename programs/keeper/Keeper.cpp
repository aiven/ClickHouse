...
try
{
    ...
    MemoryWorker memory_worker(
        config().getUInt64("memory_worker_period_ms", 0), config().getBool("memory_worker_correct_memory_tracker", false), /*use_cgroup*/ config().getBool("memory_worker_use_cgroup", true), /*page_cache*/ nullptr);
    memory_worker.start();
