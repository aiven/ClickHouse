...
# Use an explicit Atomic database so the .tmp_replace_* rename branch is exercised on every CI
...
# CREATE OR REPLACE VIEW / MATERIALIZED VIEW builds the object under an internal .tmp_replace_*
# name, registers the definer dependency there, then renames it to the target. The definer
# dependency must move with the rename, not linger on the stale .tmp_replace_* name.
