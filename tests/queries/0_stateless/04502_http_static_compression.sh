...
# Aiven: tests 5 (/binary) and 6 (/merges) are dropped -- those web UIs are not attached to
# the default HTTP handler set any more. Negotiated compression of a plain HTML web UI page
# stays covered by tests 2, 3 and 7; the pre-encoded path uses /ui instead of /clickstack.
...
echo "--- test 10: SQL Console still pre-gzipped ---"
${CLICKHOUSE_CURL} -sS -I "${BASE_URL}/ui" | grep -oF 'Content-Encoding: gzip'
...
${CLICKHOUSE_CURL} -sS -I "${BASE_URL}/ui" | grep -oF 'Vary: Accept-Encoding' || echo "no Vary (expected)"
