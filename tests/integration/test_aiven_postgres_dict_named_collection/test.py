...
def test_ddl_named_collection_with_addresses_expr_loads(started_cluster):
    """DDL named-collection + shared-parser key leg.

    A DDL-created named collection carrying ``addresses_expr`` — a key the shared
    ``StoragePostgreSQL::processNamedCollectionResult`` parser accepts but the
    dictionary's own ``dictionary_allowed_keys`` set does not — must validate and
    load rows after the patch, because the dictionary source now goes through that
    shared parser instead of a private copy. PRE-patch the same dictionary throws
    ``BAD_ARGUMENTS`` ("Unexpected key `addresses_expr`") at load time, before any
    connection is attempted.

    ``addresses_expr`` also gives dictionaries the failover address list of the
    `PostgreSQL` table engine; a single address is enough to show it is honored.
    """
    conn = get_conn(started_cluster)
    cursor = conn.cursor()
    cursor.execute("DROP TABLE IF EXISTS addresses_dict_table")
    cursor.execute("CREATE TABLE addresses_dict_table (id integer, value integer)")
    cursor.execute(
        "INSERT INTO addresses_dict_table SELECT i, i FROM generate_series(0, 9) as t(i)"
    )

    node1.query("DROP NAMED COLLECTION IF EXISTS pg_addresses_coll")
    node1.query(
        f"""
        CREATE NAMED COLLECTION pg_addresses_coll AS
            user = 'postgres',
            password = '{pg_pass}',
            addresses_expr = 'postgres1:5432',
            database = 'postgres',
            table = 'addresses_dict_table'
        """
    )

    node1.query("DROP DICTIONARY IF EXISTS pg_addresses_dict")
    node1.query(
        """
        CREATE DICTIONARY pg_addresses_dict (id UInt32, value UInt32)
        PRIMARY KEY id
        SOURCE(POSTGRESQL(NAME pg_addresses_coll))
    ...
    result = node1.query(
        "SELECT dictGetUInt32('pg_addresses_dict', 'value', toUInt64(9))"
    )
