...
def test_ch_mysql_verify_full_succeeds(started_cluster):
    ...
    Note: for MySQL the CA reaches `mysql_ssl_set` through the pre-existing
    `ssl_ca` key, which is a path and therefore comes from the `mysql_ssl` named
    collection in the server configuration file rather than from the query. The
    post-patch-only differential here is the `ssl_mode` key.
    """
    res = node.query("SELECT id FROM mysql(mysql_ssl, ssl_mode='verify-full')")
