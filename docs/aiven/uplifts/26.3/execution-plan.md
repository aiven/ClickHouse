...
**Source-branch advance (2026-06-19):** the Aiven source line advanced past our fork
point `f4552b14c77`. `git log f4552b14c77..origin/v25.8.24.21-lts-aiven` is exactly
two commits — `827d8ebb573` "Add support for protected roles" + its PR-#29 merge
`dce60488c99` (2026-06-17). This is the **only** Aiven logical patch added since the
fork point; it is captured as **row 079** in [`inventory.md`](inventory.md) (a
post-fork-point addition, outside the frozen `v25.8.18.1-lts` range) and grouped into
the Phase-3 TLS/SSL/access cohort as the sibling of 022 (protected-users). A filled
worker dispatch is prepared at `tmp/patch-079/dispatch-prompt.md`. It is the one
**open code dispatch** alongside the held clause-(v) patch 057.

**Status (refreshed 2026-06-17, audited against git):** 77 of 78 adjudicated — 66 ported (`patch-port`, incl. 023 folded into `patch-port(015)`; **004 ported 2026-06-17** as `patch-port(004)`, gated behind default-off `aiven_replace_mergetree_with_replicated`) + 11 not-carried (6 `patch-drop` 001/007/017/043/044/009; 002 & 058 dropped `obsoleted-by-upstream`, replaced by production config; 027 dropped under a `docs(...)` subject — `6a02e965f41` — not the `patch-drop` convention; **028 reclassified `still-needed-but-rewrite` (gated) and STAGED 2026-06-25** (was dropped `75dc24a6780` `obsoleted-by-upstream`; the drop missed the non-readonly CREATE path — see below); 074 dropped `obsoleted-by-upstream`, dossier-only); **1 remaining — the held clause-(v) patch 057** — see
...
Phase 3 (depends on 019). **1 patch remains:** the held clause-(v) patch **057** (004 ported 2026-06-17 as `patch-port(004)`, gated behind default-off `aiven_replace_mergetree_with_replicated`, ledger REPL-6; 058 dropped `obsoleted-by-upstream` 2026-06-17 — 26.3's `database_replicated_allow_replicated_engine_arguments=0` already rejects custom ZK path/replica name; un-bypassability restored via a production `readonly` constraint, ledger REPL-5). The Phase-4 replication core is fully committed — **066 & 078** landed as `patch-port(066,078)` (`5aa0b87b7b4`) and the `.tmp` chain landed as `patch-port(062,063,064)` (`ebe166d40f7`); 008/046/050/003/072 committed earlier; 002/009 dropped. Phases 1–3 are now complete — including the Phase-3 engine/flag toggles (052/070/071/075 ported, 074 dropped); object storage fully done (027 dropped under a `docs(...)` commit; **028 re-instated 2026-06-25 as a gated hand-authored rewrite, staged** behind default-off `aiven_skip_azure_container_creation` — the original `obsoleted-by-upstream` drop missed the non-readonly `CREATE TABLE ... ENGINE = AzureBlobStorage(...)` path, which actively probes/creates the container and hangs against the Astacus fixture's unreachable/plaintext Azurite; integration test `test_aiven_skip_azure_container_creation`). One **net-new** patch also landed, outside the 78-row inventory: `patch-new(N01)` — the `REGISTER_WEBASSEMBLY_UDF` build-time gate (see [`inventory.md`](inventory.md) "Net-new patches" and the new-surface screen). A **follow-up `patch-port(019)`** (same patch number, to be combined later) generalizes 019's two-site `query_id` reset into a process-list-level root-cause fix in `ProcessList::insert` — internal queries get a fresh id on collision instead of throwing `Code 216`, fixing the same 26.3 drift for every internal-query site (the symptom surfaced as `QUERY_WITH_SAME_ID_IS_ALREADY_RUNNING` across many tests in a downstream CI project). Ships `gtest_process_list` (fails-before/passes-after). See dossier [§2.G.2](../../patches/019-avnadmin-indirect-database-creation.md).
...
| **2** | Object storage (S3/Azure) | ~~012~~→~~013~~, ~~023~~(folded into 015)→~~024~~, ~~025~~, ~~027~~✗drop, **028→rewrite (gated, staged)**, ~~015~~→~~016~~, ~~026~~, ~~048~~ — **DONE** | 027 (`6a02e965f41`) dropped `obsoleted-by-upstream` under a `docs(...)` subject (not `patch-drop`). 028 first dropped (`75dc24a6780`) then **reclassified `still-needed-but-rewrite` (gated) and re-instated 2026-06-25**, staged behind default-off `aiven_skip_azure_container_creation` — the drop missed the non-readonly CREATE path (Astacus Azurite CREATE-hang); integration test `test_aiven_skip_azure_container_creation` |
| **2** | Kafka | ~~030~~, ~~029~~, ~~031~~, ~~032~~→~~076~~, ~~033~~ — **DONE** | — |
| **2** | Dictionaries / PostgreSQL | ~~045~~, ~~061~~, ~~036~~, ~~034~~ — **DONE** | — |
| **3** | TLS / SSL / access | ~~017~~✗drop, ~~018~~, ~~065~~, ~~067~~, ~~059~~, ~~014~~, ~~021~~, ~~019~~→~~020~~, ~~**022**~~, ~~**079**~~ (STAGED) — **022 chain DONE; 079 ported & staged, awaiting human commit** | 017 dropped (`mysql_require_secure_transport` upstream); 022 committed. **079 (protected-roles)** = `022→079`, the role-entity sibling of protected-users. **STAGED `patch-port(079)`, `still-needed-but-rewrite` (`byte_equivalent: false`).** The "clean cherry-pick" preflight forecast was wrong: 3/8 files conflicted — additive `fetched_from_remote_at_ms` drift (`Role.{h,cpp}`) + a **security-relevant** divergence in `InterpreterMoveAccessEntityQuery.cpp` (our 022 port hoisted the `MOVE USER` check above the `ON CLUSTER` dispatch; source 079 puts the `MOVE ROLE` check below → bypass). Human-ratified resolution hoists the ROLE check above the dispatch (mirrors 022). Ships stateless `9079_protected_roles` (local + `ON CLUSTER` matrix, causation pair) + integration `test_aiven_protected_roles` (4/4: ZK replication + hoisted MOVE denial). Dispatch + resolution: `tmp/patch-079/`. Dossier [`079-protected-roles.md`](../../patches/079-protected-roles.md). **Follow-up (2nd `patch-port(022)` commit): `fix checkProtectedTargets bad-cast on role targets`** — the 26.3 rewrite's `checkProtectedTargets` helper inspected every `... TO` target with the type-unsafe `tryRead<User>`, so a role in a `CREATE ROW POLICY/QUOTA/SETTINGS PROFILE ... TO` set raised `LOGICAL_ERROR` (`expected to be of type USER`); switched to `tryRead<IAccessEntity>` + the virtual `isProtected`, which also closes the 079 gap where a `PROTECTED` role in a `... TO` set escaped `PROTECTED_ACCESS_MANAGEMENT`. `09079` extended with role-target cases. Found via the 26.3 fork-handover regression set (`tmp/26-3-ci-logs/clickhouse_26_3_fork_handover.md` §B); dispatch `tmp/patch-022-checkprotectedtargets/`. |
...
| **057** | Remove all cloud-specific settings (610 loc) — **DEEP-SCREENED, HELD OPEN** (2026-06-17). Against OSS reality the 610 lines collapse to one real change (hard-disable `executable_pool` dict source); the rest is inert (cloud-only consumers / off compile-guards) or scope creep. A verbatim port **won't link on 26.3** (`cloud_mode` drift) and, being delete-based, regresses with `UNKNOWN_SETTING` (incl. **table-attach failure** from removed `shared_merge_tree_*` in stored metadata). Disposition deferred pending: *does Aiven need executable/file/library/odbc dict sources hard-off in prod?* (NO → drop; YES → minimal default-off `aiven_` setting, obsolete-not-delete). Dossier [`057-remove-cloud-specific-settings.md`](../../patches/057-remove-cloud-specific-settings.md). |
