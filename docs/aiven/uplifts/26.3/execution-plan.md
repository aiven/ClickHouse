...
| **3** | TLS / SSL / access | ~~017~~✗drop, ~~018~~, ~~065~~, ~~067~~, ~~059~~, ~~014~~, ~~021~~, ~~019~~→~~020~~, ~~**022**~~, ~~**079**~~ (STAGED) — **022 chain DONE; 079 ported & staged, awaiting human commit** | 017 dropped (`mysql_require_secure_transport` upstream); 022 committed. **079 (protected-roles)** = `022→079`, the role-entity sibling of protected-users. **STAGED `patch-port(079)`, `still-needed-but-rewrite` (`byte_equivalent: false`).** The "clean cherry-pick" preflight forecast was wrong: 3/8 files conflicted — additive `fetched_from_remote_at_ms` drift (`Role.{h,cpp}`) + a **security-relevant** divergence in `InterpreterMoveAccessEntityQuery.cpp` (our 022 port hoisted the `MOVE USER` check above the `ON CLUSTER` dispatch; source 079 puts the `MOVE ROLE` check below → bypass). Human-ratified resolution hoists the ROLE check above the dispatch (mirrors 022). Ships stateless `9079_protected_roles` (local + `ON CLUSTER` matrix, causation pair) + integration `test_aiven_protected_roles` (4/4: ZK replication + hoisted MOVE denial). Dispatch + resolution: `tmp/patch-079/`. Dossier [`079-protected-roles.md`](../../patches/079-protected-roles.md). |
