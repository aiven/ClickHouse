...
| 018 | ae35b0cc72 | 2025-12-13 | tilman.moeller@aiven.io | alex.khatskevich@aiven.io | 3 | 32 | Enforce HTTPS for URL storage and HTTPDictionarySource — **PORTED `patch-port(018)` (`(staged)`), `still-needed-but-rewrite`** (config-gated server setting `enforce_https_for_url_storage`, default `false` + non-session-overridable; enforcement extended to `StorageURLCluster` as a 4th file beyond the original 3). Ships integration test `test_aiven_enforce_https_url` (pre-patch 3 failed / post-patch 5 passed). See dossier [`018-enforce-https-url-storage.md`](../../patches/018-enforce-https-url-storage.md). | no |
