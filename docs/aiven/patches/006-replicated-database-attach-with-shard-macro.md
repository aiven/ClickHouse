...
| 26.3-aiven | (staged — pending human commit) | T3.6 worker (port) + Tilman Moeller (integration test) | byte-equivalent; cherry-pick clean; integration test added (`test_aiven_replicated_database_attach_with_shard_macro`) with verified pre/post evidence pair |
