...
| 26.3-aiven | (staged; to be filled in by human at commit time) | T3.4 worker (Claude Opus 4.7) | `byte-equivalent: false` — test renamed to `9077_*` per Aiven §4.1; context-only delta at both sites from 26.3-only `parts_in_progress[_names]` fields |
