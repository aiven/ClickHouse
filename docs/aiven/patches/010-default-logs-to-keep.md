...
| 26.3-aiven | (staged; to be filled in by human at commit time) | T3.7 worker | conflict-resolved (context-line shift); `byte_equivalent: false` per Tier 2 decomposition (only `+`/`-` lines preserved, surrounding context drifted) |
