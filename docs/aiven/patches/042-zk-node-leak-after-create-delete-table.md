...
| 26.3-aiven | (staged) | T3.15 worker | textual cherry-pick clean; Tier 2 GREEN with `byte_equivalent: false` (style-only Egyptian → Allman brace reformat; decomposition shows added/removed tokens identical modulo whitespace); stateless test with verified pre/post evidence pair |

The 26.3-aiven row stays "(staged)" until the human commits.
