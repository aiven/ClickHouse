...
Dossiers live at `docs/aiven/patches/<NNN>-<slug>.md` with `<NNN>` a **stable numeric prefix assigned at first carry**. The 71 patches that exist in the 25.8 inventory inherit `001`–`071` by chronological introduction date; new patches that land later get the next unused number (`072`, `073`, …); dropped patches keep their slot (`status: superseded` or `status: dropped-in-vXX.Y`) and the number is never reused.
...
|---|---|---|---|
| Worker silently produces a wrong cherry-pick that passes all three tiers | Low | High | (a) Schema requires `byte_equivalent` flag — non-byte-equivalent forces `semantic_conflict` escalation. (b) Tier-3 is a real test, not just compile. (c) Human review of staged state is mandatory before commit. |
...
1. **Aiven test-numbering convention.** The first patch that adds a stateless test will pick a number; we decide then whether to use next-sequential (e.g., `04206_aiven_*`) or a reserved range (e.g., `99xxx_aiven_*`). Outcome documented in the patch dossier and propagated forward.
